...
class HomeCustomizationService extends ChangeNotifier {
  ...
  Future<void> load() async {
    ...
    if (savedLeft != null) {
      _navLeft = AppFeature.values.firstWhere((f) => f.id == savedLeft, orElse: () => defaultNavLeft);
    }

    final savedRight = prefs.getString(_keyNavRight);
    if (savedRight != null) {
      _navRight = AppFeature.values.firstWhere((f) => f.id == savedRight, orElse: () => defaultNavRight);
    }
  }
